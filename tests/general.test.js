const { HTMLRx } = require('../dist/index.js');
...
describe('HTMLRx', () => {
    ...
    test('select()', () => {
        ...
        expect(T.select('ol')).toStrictEqual([['<ol class="nested-list" data-test="my-nested-list">', 489]])
        expect(T.select('h1',{class:'heading'})).toStrictEqual([['<h1 class="heading" data-test="my-heading">', 70]])
        expect(T.select('li',{'data-test':'my-item-2'})).toStrictEqual([['<li class="item" data-test="my-item-2">', 335]])
        expect(T.select('li',{'':'item'})).toStrictEqual(['<li class="item" data-test="my-item-1">'])
        expect(T.select(null,{'': 'my-container'})).toStrictEqual(['<div class="container" data-test="my-container">'])
    });

    test('element()', () => {

        expect(T.element('ol')).toBe(`<ol class="nested-list" data-test="my-nested-list">
                        <li class="nested-item" data-test="my-nested-item-1">Nested item 3.1</li>
                        <li class="nested-item" data-test="my-nested-item-2">Nested item 3.2</li>
                    </ol>`)

        expect(T.element('h1',{class:'heading'})).toBe('<h1 class="heading" data-test="my-heading">Heading 1</h1>')
